...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function(exports){
  ...
  ,thisTable = function(){
    ...
    return {
      ...
      ,exportClick: function(){
        let type = $(this).data('csv')
        table.exportFile.call(that, id, null, type);
      }
    }
  }
  ...
  Class.prototype.render = function(){
    ...
    //调整上方外边距以及对表格的整体宽度进行拉伸
    var render_div = that.layBox.parent();
    render_div.css("margin-top", "0px");
    var render_table = render_div.find('table');
    var table_width = render_table.width();
    var div_width = render_table.parent().width();
    //如果无法填满整个屏幕，则进行拉伸
    if (div_width > table_width) {
        $(".layui-table").addClass("layui-table_1");
    }

    that.adaptHeightWidth();

  };

  //固定列首的宽高
  Class.prototype.adaptHeightWidth = function () {
    var that = this;
    //适配固定表格的高度和宽度
    //对表头进行适配
    var fixed_header_trs = that.layBox.find(".layui-table-fixed .layui-table-header table tr");
    fixed_header_trs.each(function (fixed_header_trs_index, fixed_header_tr) {
      var table_header_trs = that.layBox.find(".layui-table-header  tr");
      var height = $((table_header_trs)[fixed_header_trs_index])[0].getBoundingClientRect()['height'];
      $(fixed_header_tr).height(height.toString());

      let table_header_tr = $((table_header_trs)[fixed_header_trs_index]);
      var len = $(fixed_header_tr).children().length;
      for (var i = 0; i < len; i++) {
        let fixed_header_row = $(($(fixed_header_tr).children())[i]);
        let table_header_row = $(($(table_header_tr).children())[i]);

        var width = table_header_row[0].getBoundingClientRect()['width'];
        fixed_header_row.width(width);
      }
    });

    //对body进行适配
    var fixed_body_trs = that.layBox.find(".layui-table-fixed .layui-table-body table tr");
    fixed_body_trs.each(function (fixed_body_trs_index, fixed_body_tr) {
      var table_body_trs = that.layBox.find(".layui-table-body  tr");
      var height = $((table_body_trs)[fixed_body_trs_index])[0].getBoundingClientRect()['height'];
      $(fixed_body_tr).height(height);

      let table_body_tr = $((table_body_trs)[fixed_body_trs_index]);
      var len = $(fixed_body_tr).children().length;
      for (var i = 0; i < len; i++) {
        let fixed_body_row = $(($(fixed_body_tr).children())[i]);
        let table_body_row = $(($(table_body_tr).children())[i]);

        var width = table_body_row[0].getBoundingClientRect()['width'];
        fixed_body_row.width(width);
      }
    })
  };
